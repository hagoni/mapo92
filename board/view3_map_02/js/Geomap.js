...
(function($) {
	...
	Geomap.prototype = {
		...
		opt: {
			initGeocode: {lat: 37.5645541852439, lng: 126.975613605737},
			mapOptions: {
				zoom: 14,
	...
	Geomap.prototype.initialize = function(container, options, callback) {
		...
		if(opt.getCurrentPosition === true) {
			this.getCurrentPosition(generateMap, opt.initGeocode);
		} else {
			generateMap(opt.initGeocode);
		}
	};
 ...
 ****************************************/
	...
	var Geomapping = function() {
		...
		var	options = {mapOptions: mapOptions, getCurrentPosition: true, icon: markerImg, infoWindow: infoWindow, setMarker: true};
		/*
		...
		this.initialize = function() {
			...
			gmap.initialize(container, options, function() {
				if(param_search === '') gmap.setPlaceMarkers(ajaxFilePath + '/place_find.php', {board: CONST_BOARD, sca : param_sca});
				else gmap.setPlaceMarkers(ajaxFilePath+'/place_find.php', {board: CONST_BOARD, sca : param_sca, select : param_select, search: param_search});
			});
		};
 ...
 ****************************************/
	...
	var GeomapFind = function() {
		...
		var boardWrap = $('#boardWrap'),
			listUrl = CONST_REQUEST_ROOT + '/board/index.php';
		...
		this.local1Action = function(e) {
			...
			if(typeof depth2 === 'object') {
				...
				var query = {board: CONST_BOARD, sca : param_sca, select : 'addr_road||addr_number', search: addr};
			} else {
				var query = {board: CONST_BOARD, sca : param_sca, search: ''};
			}
		...
		this.findAction = function(e) {
			...
			local2.empty();
			if(updateDisable == false){
				var query = {board: CONST_BOARD, sca : param_sca, select : 'addr_road||addr_number||title_01', search: value};
				gmap.setPlaceMarkers(requestUrl, query);
				_this.updateList(query);
		...
		this.updateList = function(query) {
			$.get(listUrl, query, function(response) {
				boardWrap.html($(response).find('#boardWrap').html());
			}, 'html').error(function(e) {
				alert(e.statusText);
			});;
		};
