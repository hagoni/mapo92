...
(function($) {
	...
	window.Counting = function(elems, options) {
        ...
        var opt = {type: 'text', duration: 100, delay: 1000, interval: 5000, repeat: 0, loop: 1, diff: 100, min: 0, max: 10, slowFx: false, slowV: 10, anim: false};

		var	numbers = [], repeatTimer = null, slotTimer = [], repeat = 0, distance, slowlyI = 0, animTl = [];
        ...
        this.queue = function(i, j) {
            ...
            slotTimer[k] = setTimeout(function() {
                opt.type === 'text' ? elems.eq(i).text(numbers[i][j]) : elems.eq(i).css({backgroundPositionY: -(opt.diff * numbers[i][j])});
                if(k === limit) {
                    if(opt.repeat === 1 || repeat < opt.repeat - 1) {
                        _this.setTimer();
                        repeat++;
    ...
    doc.ready(function() {
		...
		new YMotion([
			[
				...
				{el: '.el1_4', set: {backgroundPositionY: 0}, to: {backgroundPositionY: -111}, d: 0.4},
				{el: '.el1_4', set: {backgroundPositionY: 0}, to: {backgroundPositionY: -222}, d: 0.4, t: '+=0.5'},
				{el: '.el1_4', set: {backgroundPositionY: 0}, to: {backgroundPositionY: -333}, d: 0.4, t: '+=0.5'},
				{el: '.el1_4', set: {backgroundPositionY: 0}, to: {backgroundPositionY: -444}, d: 0.4, t: '+=0.5'},
			],
			[
			...
			[
				...
				{el: '.el4_1', set: {opacity: 1}, to: {opacity: 0, repeat: -1, repeatDelay: 0.3, yoyo: true}, d: 0.1}
			],
			[
