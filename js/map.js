...
(function($) {
	...
	Geomap.prototype = {
		...
		markers: [],
		opt: {
			initGeocode: {lat: 37.5556326132925, lng: 126.992217614712},
			mapOptions: {
				zoom: 12,
		...
		if(window.location.protocol.indexOf('https') > -1) {
			...
			if(typeof navigator.geolocation === 'object') {
				navigator.geolocation.getCurrentPosition(function(response) {
					// geocode.lat = response.coords.latitude;
					// geocode.lng = response.coords.longitude;
...
(function($) {
    $(document).ready(function() {
        ...
        $geomap.initialize('placeLoadMap', {
            icon: CONST_ROOT + '/design/other/marker_big.png'
        }, function() {
            setMarkers({});
        });
